...
int main(int argc, const char *argv[])
{
    ...
    log_file << ",,,,,,,," << endl;
    log_file << "TTC Camera,,,Detectors,,,,," << endl;
    log_file << ",,,HARRIS,FAST,BRISK,ORB,AKAZE,SIFT" << endl;
    log_file << ",,,,,,,," << endl;
    log_file << "Descriptors,,,,,,,," << endl;

    /* MAIN LOOP OVER ALL IMAGES */
    for (string descriptor_type: v_descriptor_type) {
        for (string detector_type: v_detector_type) {

            log_file << "," << descriptor_type << detector_type << ",ImgIndex,,,,,," << endl;// l.5
            ...
            for (size_t imgIndex = 0; imgIndex <= imgEndIndex - imgStartIndex; imgIndex+=imgStepWidth)
            {
                ...
                if (dataBuffer.size() > 1) // wait until at least two images have been processed
                {
                    ...
                    if (descriptor_type == "SIFT") {
                        string descriptor_type = "DES_HOG";
                    } else {
                    ...
                    for (auto it1 = (dataBuffer.end() - 1)->bbMatches.begin(); it1 != (dataBuffer.end() - 1)->bbMatches.end(); ++it1)
                    {
                        ...
                        if( currBB->lidarPoints.size()>0 && prevBB->lidarPoints.size()>0 ) // only compute TTC if we have Lidar points
                        {
                            ...
                            if(bVis)
                            {
                                ...
                                v_bb.push_back(*prevBB);
                                v_bb.push_back(*currBB);
                                show3DObjects(v_bb, cv::Size(4.0, 20.0), cv::Size(2000, 2000), true);
                            }
                            bVis = false;
